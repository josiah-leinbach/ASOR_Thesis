...
```{r}
paul_pos.list_full <- readRDS("paul_6pos-HJ123.rds")
paul_pos.list <- paul_pos.list_full[-c(16,17)]
...
for (i in 1:length(seed.list)) {
  file.names[i] <- paste0("EMR.HJ1-R1.5-R2.3_",seed.list[i],".rds")
}
...
pi_reg <- c(0.25, 0.15, 0.10, 0.10, 0.10, 0.10, 0.10, 0.10)
for (i in 1:R1){
  B[i,i] <- 1
}
...
for (i in 1:R) {
  for (j in 1:R) {
    counter <- numeric(0)
    for (k in 1:(N[14]-1)) {
      if (r.mn_id[14,k] == i && r.mn_id[14,k+1] == j) {
        counter <- append(counter, 1)
      }
    }
    A[i,j] <- A[i,j] + sum(counter)
...
for (i in 1:R) {
  for (j in 1:R) {
    counter <- numeric(0)
    for (k in 1:(N[15]-1)) {
      if (r.mn_id[15,k] == i && r.mn_id[15,k+1] == j) {
        counter <- append(counter, 1)
      }
    }
    A[i,j] <- A[i,j] + sum(counter)
...
colnames(A) <- c("R1","R2","R3","R4","R5","R6","R7","R8")
rownames(A) <- c("R1","R2","R3","R4","R5","R6","R7","R8")
...
r6 <- vector(mode = "list", length = 0)
r7 <- vector(mode = "list", length = 0)
r8 <- vector(mode = "list", length = 0)
...
for (i in 1:L) {
  ...
  r6 <- append(r6, book[r.mn_id[i,1:length(paul_pos.list[[i]])] == 6])
  r7 <- append(r7, book[r.mn_id[i,1:length(paul_pos.list[[i]])] == 7])
  r8 <- append(r8, book[r.mn_id[i,1:length(paul_pos.list[[i]])] == 8])
}
...
for (i in 1:length(r6)) {
  r6_vec[i] <- as.character(r6[[i]][1])
}
r6_vec <- factor(r6_vec)

r7_vec <- numeric(0)
for (i in 1:length(r7)) {
  r7_vec[i] <- as.character(r7[[i]][1])
}
r7_vec <- factor(r7_vec)

r8_vec <- numeric(0)
for (i in 1:length(r8)) {
  r8_vec[i] <- as.character(r8[[i]][1])
}
r8_vec <- factor(r8_vec)


# Apply to matrix
r_list <- vector(mode = "list", length = 7)
r_list[[1]] <- as.character(r1_vec); r_list[[2]] <- as.character(r2_vec); r_list[[3]] <- as.character(r3_vec); r_list[[4]] <- as.character(r4_vec); r_list[[5]] <- as.character(r5_vec); r_list[[6]] <- as.character(r6_vec);
r_list[[7]] <- as.character(r7_vec); r_list[[8]] <- as.character(r8_vec)
...
rownames(Alpha) <- c("R1", "R2", "R3", "R4", "R5", "R6", "R7", "R8")
...
# Regime 6
regime_6 <- matrix(data = NA, K, K)

for (i in 1:K) {
  for (j in 1:K) {
    counter <- numeric(0)
    for (k in 1:(length(r6))) {
      r_work <- r6[[k]]
      if (length(r_work) > 1) {
        for (l in 1:(length(r_work)-1)) {
          if (r_work[l] == stateList[i] && r_work[l+1] == stateList[j]) {
      ...
      regime_6[i,j] <- sum(counter)
    }
  }
}

# Regime 7
regime_7 <- matrix(data = NA, K, K)

for (i in 1:K) {
  for (j in 1:K) {
    counter <- numeric(0)
    for (k in 1:(length(r7))) {
      r_work <- r7[[k]]
      if (length(r_work) > 1) {
        for (l in 1:(length(r_work)-1)) {
          if (r_work[l] == stateList[i] && r_work[l+1] == stateList[j]) {
      ...
      regime_7[i,j] <- sum(counter)
    }
  }
}

# Regime 8
regime_8 <- matrix(data = NA, K, K)

for (i in 1:K) {
  for (j in 1:K) {
    counter <- numeric(0)
    for (k in 1:(length(r8))) {
      r_work <- r8[[k]]
      if (length(r_work) > 1) {
        for (l in 1:(length(r_work)-1)) {
          if (r_work[l] == stateList[i] && r_work[l+1] == stateList[j]) {
...
Gamma_array[6,,] <- round(regime_6/sum(regime_6),5)
Gamma_array[7,,] <- round(regime_7/sum(regime_7),5)
Gamma_array[8,,] <- round(regime_7/sum(regime_8),5)
