...
# c(1092,1169,1214,1485,1580,1587,1603,1625,1760,1763,1779,1868,1914,1918,
#               1942,1944,1946,1951,1952,1979)
# Already complete: 1649, 1837, 1916, 1939, 1940

file.names <- numeric(0) 
for (i in 1:length(seed.list)) {
  file.names[i] <- paste0("EMR.HJ1-R1.1-R2.3_", seed.list[i], ".rds")
}

BIC.HJ1_6pos_r1.2_r2.2 <- numeric(length(seed.list))
```

# R1 = 2, R2 = 2, POS = 6, 7 Undisputed
```{r}
for (w in 1:25) {
...
# Apply to matrix
r_list <- vector(mode = "list", length = 4)
r_list[[1]] <- as.character(r1_vec); r_list[[2]] <- as.character(r2_vec); r_list[[3]] <- as.character(r3_vec); r_list[[4]] <- as.character(r4_vec)
...
BIC.HJ1_6pos_r1.2_r2.2[w] <- EMR$BIC
