...
for (i in 1:length(seed.list)) {
  file.names[i] <- paste0("EMR.HJ1-R1.2-R2.4_",seed.list[i],".rds")
}
...
pi_reg <- c(0.45, 0.25, 0.075, 0.075, 0.075, 0.075)
for (i in 1:R1){
  B[i,i] <- 1
}
...
colnames(A) <- c("R1","R2","R3","R4","R5","R6")
...
# Apply to matrix
r_list <- vector(mode = "list", length = 6)
...
rownames(Alpha) <- c("R1", "R2", "R3", "R4", "R5", "R6")
