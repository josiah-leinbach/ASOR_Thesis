...
seed.list <- c(597,731,1066,1170,1215,1348,1415,1534,1556,1588,
...
for (i in 1:length(seed.list)) {
  file.names[i] <- paste0("EMR.HJ123-R1.3-R2.1_NEW_",seed.list[i],".rds")
}
...
for (i in 1:L) {
  ...
  r4 <- append(r3, book[r.mn_id[i,1:length(paul_pos.list[[i]])] == 4])
}
...
for (i in 1:4) {
  for (j in 1:length(stateList)) {
    ...
    Alpha[i,j] <- (sum(r_list[[i]] == q))/length(r_list[[i]])
  }
}
...
Gamma_array[4,,] <- round(regime_3/sum(regime_4),5)
