...
```{r}
paul_pos.list <- readRDS("paul_6pos-HJ123.rds")
...
for (i in 1:length(seed.list)) {
  file.names[i] <- paste0("EMR.HJ123-R1.2-R2.2_NEW_",seed.list[i],".rds")
}
...
for (i in 1:R) {
  for (j in 1:R) {
    counter <- numeric(0)
    for (k in 1:(N[16]-1)) {
      if (r.mn_id[16,k] == i && r.mn_id[16,k+1] == j) {
        counter <- append(counter, 1)
      }
    }
    A[i,j] <- A[i,j] + sum(counter)
...
for (i in 1:R) {
  for (j in 1:R) {
    counter <- numeric(0)
    for (k in 1:(N[17]-1)) {
      if (r.mn_id[17,k] == i && r.mn_id[17,k+1] == j) {
        counter <- append(counter, 1)
      }
    }
    A[i,j] <- A[i,j] + sum(counter)
