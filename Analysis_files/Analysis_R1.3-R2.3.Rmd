...
pi_reg <- c(0.45, 0.25, 0.1, 0.1, 0.05, 0.05)
for (i in 1:R1){
  B[i,i] <- 1
}
...
## Adjust for R4, R5, and R6 not having any I POS tags
r_list <- vector(mode = "list", length = 3)
r_list[[1]] <- as.character(r4_vec); r_list[[2]] <- as.character(r5_vec); r_list[[3]] <- as.character(r6_vec)

for (i in 1:3) {
  for (j in 1:length(stateList)) {
    q <- as.character(stateList[j])
    Alpha[i+3,j] <- (sum(r_list[[i]] == q))/length(r_list[[i]])
  }
}
...
for (i in 1:K) {
  for (j in 1:K) {
    counter <- numeric(0)
    for (k in 1:(length(r6_vec)-1)) {
      if (r6_vec[k] == stateList[i] && r6_vec[k+1] == stateList[j]) {
        counter <- append(counter, 1)
      }
...
saveRDS(EMR,"EMR-R1.3-R2.3-9pos.rds")
```
