...
# R1 = 2, R2 = 1, POS = 6, 7 Undisputed Minus 1 Thessalonians
```{r}
paul_pos.list <- readRDS("paul_9pos.rds")
paul_pos.list <- c(paul_pos.list[c(-4)], paul_pos.list[4])
...
pi_reg <- c(0.5, 0.25, 0.25) ## Changed from (0.5, 0.3, 0.2) to reflect new distribution of undisputed
for (i in 1:R1){
  B[i,i] <- 1
}
...
saveRDS(EMR,"EMR-R1.2-R2.1-9pos_Test.Gal.rds")
```
