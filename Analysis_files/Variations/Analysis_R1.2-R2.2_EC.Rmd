...
## Adjust for R3 and R4 not having any I POS tags
r_list <- vector(mode = "list", length = 2)
r_list[[1]] <- as.character(r3_vec); r_list[[2]] <- as.character(r4_vec)

for (i in 1:2) {
  for (j in 1:length(stateList)) {
    q <- as.character(stateList[j])
    Alpha[i+2,j] <- (sum(r_list[[i]] == q))/length(r_list[[i]])
  }
}
...
saveRDS(EMR,"EMR-R1.2-R2.2-9pos_EC.rds")
```
